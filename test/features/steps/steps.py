...
def active_truncated_facet_visible(context, title, truncated_title):
    context.execute_steps(u"""
        When I reload page every 2 seconds until I see an element with xpath "//li[contains(@class, 'nav-item') and contains(@class, 'active')]//a[contains(@title, '{title}') or contains(@data-bs-title, '{title}')]" but not more than 5 times
