...
def truncated_facet_visible(context, title, truncated_title):
    context.execute_steps(u"""
        When I reload page every 2 seconds until I see an element with xpath "//li[contains(@class, 'nav-item')]//a[starts-with(@title, '{title}') or starts-with(@data-bs-title, '{title}')]" but not more than 5 times
